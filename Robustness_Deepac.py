...
if __name__ == "__main__":
    ...
    y_o = load_pkl("/home/family/Videos/deepac/kmer/y_kmer.pkl")
    s_o = np.load("/home/family/Videos/deepac/kmer/kmer_deepac.txt.npy")

    y_s = np.load("/home/family/Videos/eval/strain level/path_test/y_valid.npy")
    s_s = np.load("/home/family/Videos/deepac/kmer/sim_deepac.txt.npy")

    y_r = load_pkl("/home/family/Videos/deepac/kmer/y_kmer.pkl")
    s_r = load_pkl("/home/family/Videos/deepac/kmer/Rev_kmer_deepac.pkl")
    s_o = s_o.squeeze()
    s_s = s_s.squeeze()
    s_r = s_r.squeeze()
    print(s_r.shape)
    ...
    # -----------------------------------------------------
    sim_df_2.to_csv(
        "simulated_bootstrap_results_2_deepac.csv",
        index=False
    )

    rev_df_2.to_csv(
        "reverse_complement_bootstrap_results_2_deepac.csv",
        index=False
    )
    ...
    effect_sim_df_2.to_csv(
        "effect_sizes_simulated_2_deepac.csv",
        index=False
    )

    effect_rev_df_2.to_csv(
        "effect_sizes_revcomp_2_deepac.csv",
        index=False
    )
    ...
    wasserstein_df_2.to_csv(
        "wasserstein_shift_results_2_deepac.csv",
        index=False
    )
    ...
    forest_plot(
        sim_results,
        "Simulated Reads Difference_2_deepac",
        is_difference_plot=True
    )

    forest_plot(
        rev_results,
        "Reverse Complement Difference_2_deepac",
        is_difference_plot=True
    )
